...
This is the point where you need a **brutal design review** before spending days in the Cadence lab.
...
This is exactly how reviewers expect architectural ablation studies.
...
Most student systolic arrays ignore timing alignment.

The fact that you explicitly identified:
...
15 rows × 3 cycles
=
45-cycle alignment network
```

Reviewers may ask:

> What is the area/power cost of the skew registers?

You must measure this later.

Do not hide it.

---

### 2. Direct Weight Injection
...
This is fine for RTL verification.
...
For the paper, it is not enough.

---

### 3. B16 Is Not Yet Proven
...
```text
❌ RTL simulation not run
❌ Waveforms not verified
```

Therefore currently:

```text
B16 status = unverified
```
...
| Architecture         | 90%    |
| RTL                  | 85%    |
| Verification Concept | 95%    |
| Actual Simulation    | 0%     |
| Waveform Validation  | 0%     |
| Synthesis            | 0%     |
| Power Results        | 0%     |
| Publishable Evidence | 0%     |

So your next milestone is **not new RTL**.
...
If B16 passes simulation and synthesizes cleanly, then the architecture is strong enough to move into HSA-PE development. Until then, treat every architectural claim as provisional.


Right now, **do not write another line of RTL**.

You have reached the point where architecture work must stop and validation must start.
...
```bash
git remote add origin https://github.com/aashishniranjanb/HSA16-AI-Accelerator.git

git push -u origin feature/b16-baseline
```

Goal:

```text
All Phase-1 files safely stored.
```

---

## 2. Go to the Lab and Run PE First
...
After lab you will have one of two outcomes.
...
(add only zero detect + clock gating + operand isolation)
...
1. Push GitHub
2. Run PE simulation
3. Run B16 simulation
4. Verify waveform
5. Run Genus
6. Collect area/timing reports
```

Do not start row gating, tile gating, DFT, ResNet, MobileNet, or power analysis until B16 is proven working. A verified baseline is worth far more than three unfinished novelty blocks.
