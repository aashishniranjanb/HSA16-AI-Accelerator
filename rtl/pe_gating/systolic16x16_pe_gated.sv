//============================================================
// Systolic 16×16 with PE Gating — Weight-Stationary Array
//============================================================

`include "hsa_params.svh"

module systolic16x16_pe_gated
(
    input  logic                                 clk,
    input  logic                                 rst_n,
    ...
    // Gating Statistics
    //------------------------------------------------------
    output logic [31:0]                          total_gated_macs,
    output logic [31:0]                          total_executed_macs
);
    ...
    // Connectivity wires for statistics counters
    logic [31:0] pe_gated_cnt     [0:ARRAY_SIZE-1][0:ARRAY_SIZE-1];
    logic [31:0] pe_executed_cnt  [0:ARRAY_SIZE-1][0:ARRAY_SIZE-1];
        ...
        for (gr = 0; gr < ARRAY_SIZE; gr = gr + 1) begin : gen_pe_row
            for (gcol = 0; gcol < ARRAY_SIZE; gcol = gcol + 1) begin : gen_pe_col
                pe_gated u_pe (
                    ...
                    // Statistics
                    .gated_mac_cnt    (pe_gated_cnt[gr][gcol]),
                    .executed_mac_cnt (pe_executed_cnt[gr][gcol])
                );
            end
        end
    endgenerate
    ...
    //----------------------------------------------------------
    // 7. Sum Statistics Counters
    //----------------------------------------------------------
    always_comb begin
        total_gated_macs = '0;
        total_executed_macs = '0;
        for (int r = 0; r < ARRAY_SIZE; r++) begin
            for (int c = 0; c < ARRAY_SIZE; c++) begin
                total_gated_macs = total_gated_macs + pe_gated_cnt[r][c];
                total_executed_macs = total_executed_macs + pe_executed_cnt[r][c];
            end
        end
    end
