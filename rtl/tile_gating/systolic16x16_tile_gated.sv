//============================================================
// Systolic 16×16 with Tile Gating — Weight-Stationary Array
//============================================================

`include "hsa_params.svh"

module systolic16x16_tile_gated
(
    input  logic                                 clk,
    input  logic                                 rst_n,
    ...
    // Gating Statistics
    //------------------------------------------------------
    output logic [31:0]                          tile_skip_counter
);
    ...
    logic tile_has_valid  [0:3][0:3];

    // 3-cycle shift registers to propagate tile gating enable horizontally
    logic tile_active_d1  [0:3][0:3][0:2];
    logic tile_active_d2  [0:3][0:3][0:2];
    logic tile_active_d3  [0:3][0:3][0:2];

    generate
        for (genvar tr = 0; tr < 4; tr = tr + 1) begin : gen_tile_rows
            for (genvar tc = 0; tc < 4; tc = tc + 1) begin : gen_tile_cols
                // Evaluation of tile inputs at its left boundary
                assign tile_has_valid[tr][tc] = valid_wire[tr*4][tc*4]   || valid_wire[tr*4+1][tc*4]   ||
                                                valid_wire[tr*4+2][tc*4] || valid_wire[tr*4+3][tc*4];
                ...
                always_ff @(posedge clk or negedge rst_n) begin
                    if (!rst_n) begin
                        tile_active_d1[tr][tc][0] <= 1'b0;
                        tile_active_d1[tr][tc][1] <= 1'b0;
                        tile_active_d1[tr][tc][2] <= 1'b0;
                    end else begin
                        tile_active_d1[tr][tc][0] <= tile_active_raw[tr][tc];
                        tile_active_d1[tr][tc][1] <= tile_active_d1[tr][tc][0];
                        tile_active_d1[tr][tc][2] <= tile_active_d1[tr][tc][1];
                    end
                end
                ...
                always_ff @(posedge clk or negedge rst_n) begin
                    if (!rst_n) begin
                        tile_active_d2[tr][tc][0] <= 1'b0;
                        tile_active_d2[tr][tc][1] <= 1'b0;
                        tile_active_d2[tr][tc][2] <= 1'b0;
                    end else begin
                        tile_active_d2[tr][tc][0] <= tile_active_d1[tr][tc][2];
                        tile_active_d2[tr][tc][1] <= tile_active_d2[tr][tc][0];
                        tile_active_d2[tr][tc][2] <= tile_active_d2[tr][tc][1];
                    end
                end
                ...
                always_ff @(posedge clk or negedge rst_n) begin
                    if (!rst_n) begin
                        tile_active_d3[tr][tc][0] <= 1'b0;
                        tile_active_d3[tr][tc][1] <= 1'b0;
                        tile_active_d3[tr][tc][2] <= 1'b0;
                    end else begin
                        tile_active_d3[tr][tc][0] <= tile_active_d2[tr][tc][2];
                        tile_active_d3[tr][tc][1] <= tile_active_d3[tr][tc][0];
                        tile_active_d3[tr][tc][2] <= tile_active_d3[tr][tc][1];
                    end
                end
            end
        end
    ...
    // Route tile gating enables to PEs using compile-time generate routing
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (int tr = 0; tr < 4; tr++) begin
                for (int tc = 0; tc < 4; tc++) begin
                    if (tile_has_valid[tr][tc] && !tile_active_raw[tr][tc]) begin
                        tile_skip_counter <= tile_skip_counter + 1'b1;
                    end
                end
            end
        end
