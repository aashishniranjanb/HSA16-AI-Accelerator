//============================================================
// TB B16 Hierarchical Gating — Systolic Array Verification
//============================================================

`timescale 1ns/1ps

module tb_b16_hierarchical;
    ...
    systolic16x16_hierarchical dut
    (
        .clk              (clk),
        .rst_n            (rst_n),
    ...
    begin
        ...
        $display("");
        $display("================================================");
        $display(" B16 HIERARCHICAL GATED ARRAY VERIFICATION START");
        $display("================================================");
        $display("");
        ...
        //------------------------------------------------------
        // Run Workloads and print statistics
        //------------------------------------------------------
        $display("\n==================================================================================");
        $display(" HIERARCHICAL GATING WORKLOAD CHARACTERIZATION");
        $display("==================================================================================");
        $display("Format: Workload | Tile Gated | Row Gated | PE Gated | Executed | Total MACs");
        $display("----------------------------------------------------------------------------------");

        // 1. Dense
        run_workload("dense");
        $display("Dense       | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 2. Sparse50
        run_workload("sparse50");
        $display("Sparse50    | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 3. Sparse70
        run_workload("sparse70");
        $display("Sparse70    | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 4. Sparse90
        run_workload("sparse90");
        $display("Sparse90    | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 5. Sparse95
        run_workload("sparse95");
        $display("Sparse95    | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 6. AlexNet
        run_workload("alexnet");
        $display("AlexNet     | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 7. VGG16
        run_workload("vgg16");
        $display("VGG16       | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);

        // 8. ResNet18
        run_workload("resnet18");
        $display("ResNet18    | %10d | %9d | %8d | %8d | %10d", 
            total_tile_gated, total_row_gated, total_pe_gated, total_exec, 4096);
        ...
        if (total_fail == 0)
            $display("HIERARCHICAL_GATED_B16_TEST PASSED");
        else
            $display("HIERARCHICAL_GATED_B16_TEST FAILED");
