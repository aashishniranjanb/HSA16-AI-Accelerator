//============================================================
// TB B16 Tile Gated — 16×16 Systolic Array Verification
//============================================================

`timescale 1ns/1ps

module tb_b16_tile_gated;
    ...
    (
        ...
        .tile_skip_counter (tile_skip_counter)
    );
    ...
    begin
        ...
        $display("");
        $display("================================================");
        $display(" B16 TILE GATED ARRAY VERIFICATION START");
        $display("================================================");
        $display("");
        ...
        //------------------------------------------------------
        $display("\n================================================");
        $display(" TILE GATING WORKLOAD CHARACTERIZATION");
        $display("================================================");

        run_workload("dense");
        $display("Dense    : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("sparse50");
        $display("Sparse50 : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("sparse70");
        $display("Sparse70 : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("sparse90");
        $display("Sparse90 : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("sparse95");
        $display("Sparse95 : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("alexnet");
        $display("AlexNet  : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("vgg16");
        $display("VGG16    : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("resnet18");
        $display("ResNet18 : Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("mobilenetv2");
        $display("MobileNetV2: Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);

        run_workload("efficientnet_b0");
        $display("EfficientNet: Skipped Tile-wavefront Cycles = %6d (Total Tile-wavefronts evaluated = %d)", tile_skip_counter, 64);
        ...
        if (total_fail == 0)
            $display("TILE_GATED_B16_TEST PASSED");
        else
            $display("TILE_GATED_B16_TEST FAILED");
    ...
    initial
    begin
        $dumpfile("flow/xrun/tile_gating.vcd");
        $dumpvars(0, tb_b16_tile_gated);
    end
