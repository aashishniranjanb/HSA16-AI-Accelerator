//============================================================
// TB B16 Row Gated — 16×16 Systolic Array Verification
//============================================================

`timescale 1ns/1ps

module tb_b16_row_gated;
    ...
    (
        ...
        .row_skip_counter (row_skip_counter)
    );
    ...
    begin
        ...
        $display("");
        $display("================================================");
        $display(" B16 ROW GATED ARRAY VERIFICATION START");
        $display("================================================");
        $display("");
        ...
        //------------------------------------------------------
        $display("\n================================================");
        $display(" ROW GATING WORKLOAD CHARACTERIZATION");
        $display("================================================");

        run_workload("dense");
        $display("Dense    : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("sparse50");
        $display("Sparse50 : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("sparse70");
        $display("Sparse70 : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("sparse90");
        $display("Sparse90 : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("sparse95");
        $display("Sparse95 : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("alexnet");
        $display("AlexNet  : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("vgg16");
        $display("VGG16    : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);

        run_workload("resnet18");
        $display("ResNet18 : Skipped Row-wavefront Cycles = %6d (Total Row-wavefronts evaluated = %d)", row_skip_counter, 256);
        ...
        if (total_fail == 0)
            $display("ROW_GATED_B16_TEST PASSED");
        else
            $display("ROW_GATED_B16_TEST FAILED");
